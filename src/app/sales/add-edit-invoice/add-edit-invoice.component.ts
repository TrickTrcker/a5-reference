...
export class AddEditInvoiceComponent implements OnInit, OnChanges, OnDestroy { //SEDT
  ...
  GenarateInvoice(flag) {
    ...
    if (validdata) {
      ...
      _.forEach(this.accconfig, (config) => {
        ...
        config.createddt = this.dateFormatPipeFilter.transform(new Date(), this.date_apiTSformat);
        ...
        .then(function (res) {
          ...
          if (res.status) {
            ...
            if (flag) {
              ...
              self.router.navigate(['sales/viewinvoice', 'invoice', res.data.header.invoiceid]);
            }
          }
          else {
            self.messageservice.showMessage({
