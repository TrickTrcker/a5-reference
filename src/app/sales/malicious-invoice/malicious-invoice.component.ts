...
@Component({
  selector: 'app-malicious-invoice',
  templateUrl: './malicious-invoice.component.html',
  styleUrls: ['./malicious-invoice.component.scss']
})
export class MaliciousInvoiceComponent implements OnInit, OnChanges, OnDestroy { //SEDT
  ...
  getinitialized() {
    ...
    this.featureservice.getcodemasterList(placeofsuppliesreq)
      .then(function (res) {
        ...
        self.placeofsupplies = self.masterservice.formatDataforDropdown("name", fdata, "Select Type");
      ...
      res => {
        if (res.status) {
          ...
          if (this.settinglist.length > 0) {
            ...
            console.log("PLACE_OF_SUPPLY", this.orgpos)
          }
        }
  ...
  getselectedCustomer(item: any) {
    ...
    if (!_.isEmpty(item)) {
      ...
      this.editableBillingaddress += billcityname;
      this.editableBillingaddress += billstatename;
      this.editableBillingaddress += billcountryname;
      this.editableBillingaddress += billzipcode;
  ...
  invoiceProductTotal() {
    ...
    _.forEach(this.selectedproductsList, function (item, key) {
      console.log("selected product list", item)
  ...
  GenarateInvoice(flag) {
    ...
    if (_.isEmpty(this.invoiceno) && this.autogenyn === 'N') {
      this.messageservice.showMessage({
        severity: 'error', summary: 'Error',
        detail: PrimengConstant.INVOICE.ADDEDITFORM.INVOICE_NO
      });
      return;
    }
    ...
    if (validdata) {
      ...
      for (var i = this.selectedproductsList.length - 1; i >= 0; i--) {
        ...
        && (this.selectedproductsList[i].prodname != "") && (this.selectedproductsList[i].prodname != "Select Product")) { //SEDT start
          ...
          var productItem = {
            ...
            "hsncode": this.selectedproductsList[i].hsncode //SEDT
          };
      ...
      if(this.autogenyn == 'N'){
        formData.header.invoiceno = this.invoiceno;
      }
    }
    if (validdata) {
      var self = this;
      this.salesservice.InvoiceCreate(formData)
        .then(function (res) {
          // self.msgs = [];
          if (res.status) {
            ...
            if (flag) {
              ...
              self.router.navigate(['sales/viewmiscellaneousinvoice', 'cai_invoice', res.data.header.invoiceid]);
            }
          }
          else {
            self.messageservice.showMessage({
