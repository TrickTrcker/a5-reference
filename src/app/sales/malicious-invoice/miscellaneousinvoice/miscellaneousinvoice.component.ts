import { Component, OnInit, ViewEncapsulation, ChangeDetectorRef,OnChanges, SimpleChanges } from '@angular/core';
...
@Component({
  selector: 'app-miscellaneousinvoice',
  templateUrl: '../malicious-invoice.component.html',
  styleUrls: ['./miscellaneousinvoice.component.scss']
})
export class MiscellaneousinvoiceComponent implements OnInit, OnChanges {
  ...
  Invoicecategory : any = "goodsInvoice";
  Taxeditable : boolean = false;
  editmode : boolean = true;
  hotkeySave: Hotkey | Hotkey[];
  hotkeyProduct: Hotkey | Hotkey[];
  hotkeyContact: Hotkey | Hotkey[]; //SEDT
  constructor(
    ...
    if(this.hsncodelist.length == 0 )
    {
      ...
      .then((res) => {
        if (res.status) {
          ...
          // this.hsncodelist = this.masterservice.formatDataforDropdown("gstpercent", res.data, "Select Tax");
          this.hsncodelist = this.masterservice.productHsnCodeDropdown("hsncode","description", res.data, "Select HSN Code");
        }
  ...
  setSelectedTax(event, selectedproduct, index) {
    var item : any = event.value;
    if (item.taxname == '--') {
  ...
  producteditingObserve(event, selectedproduct, index) {
    if (!_.isEmpty(selectedproduct) && selectedproduct != undefined) {
      if ((selectedproduct.prodid != 0 && selectedproduct.prodid != undefined) || (this.Invoicecategory == "ServiceInvoice") ) {
  ...
  calculateTotalTax() {
    ...
    _.forEach(this.selectedproductsList, function (item, key) {
      ...
      //   console.log("iftax",tax)
      // } else {
      //   tax += ((total * item.taxpercent) / 100);
      //   console.log("elsetax",tax)
      // }
    });
  ...
  GenarateInvoice(flag) {
    ...
    var prod_count = _.filter(this.selectedproductsList, (pro)=> {
      return ((pro.prodid != 0) || (this.Invoicecategory == "ServiceInvoice") && (parseInt(pro.quantity) > 0) );
    });
    ...
    if (validdata) {
      ...
      for (var i = this.selectedproductsList.length - 1; i >= 0; i--) {
        ...
        && (this.selectedproductsList[i].prodname != "") && (this.selectedproductsList[i].prodname != "Select Product") ) {
      ...
      var formData = {
        "header": {
          ...
          // "seqid": 77,
          "refkey": "INV",
          "revcharge": this.selectedreversecharge,
      ...
      this.salesservice.InvoiceUpdate(formData)
        .then(function (res) {
          ...
          if (res.status) {
            ...
            self.router.navigate(['sales/viewmiscellaneousinvoice', 'cai_invoice', self.invoiceid]);
            // if (flag) {
            //   self.clearform();
            // } else {
            //   self.router.navigate(['sales/viewinvoice', 'invoice', self.invoiceid]);
            // }
          }
          else {
            self.messageservice.showMessage({
  ...
  taxitemSeparation() {
    ...
    for (var i = this.selectedproductsList.length - 1; i >= 0; i--) {
      ...
      if ((this.selectedproductsList[i].prodid != 0)|| (this.Invoicecategory == "ServiceInvoice") ) {
        var ptotal = parseFloat(this.selectedproductsList[i].quantity) * parseFloat(this.selectedproductsList[i].mrp);
        var discntvalue = this.calcdiscAmount(i);
        var taxval = ((ptotal - discntvalue) * parseFloat(this.selectedproductsList[i].taxpercent)) / 100;
        discountTotal += discntvalue;
  ...
  changesonInvoiceCategoryProductBased()
  {
    this.loaddefaultproducts();
    this.ProductTotal = 0.0;
    this.discounttotal = 0.0;
