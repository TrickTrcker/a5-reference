...
@Component({
  selector: 'app-proforma-invoice-addedit',
  templateUrl: './proforma-invoice-addedit.component.html',
  styleUrls: ['./proforma-invoice-addedit.component.scss']
})
export class proformaInvoiceAddeditComponent implements OnInit, OnChanges, OnDestroy {
  ...
  {
    var self = this;
    //add_parites start -- removed direct load customer api call
    // load customer 
        ...
        && (this.selectedproductsList[i].prodname != "") && (this.selectedproductsList[i].prodname != "Select Product")) { //SEDT start
          ...
          if (this.selectedproductsList[i].taxname != "--"
            ...
            if (this.selectedproductsList[i].taxname == "--") {
              ...
              if (this.selectedproductsList[i].quantity == 0) {
                validdata = false;
        ...
        .then(function (res) {
          if (res.status) {
            ...
            if (flag) {
              ...
              self.router.navigate(['sales/viewinvoice', 'proforma', res.data.header.invoiceid]);
            }
          }
          else {
            self.messageservice.showMessage({
