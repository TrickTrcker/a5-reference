import { Component, OnInit, ViewEncapsulation, OnDestroy, ChangeDetectorRef, OnChanges, SimpleChanges,ViewChild } from '@angular/core'; //SEDT
...
@Component({
  selector: 'app-edit',
  templateUrl: '../bill-edit/bill-edit.component.html',
  styleUrls: ['../bill-edit/bill-edit.component.scss']
})
export class EditComponent implements OnInit, OnChanges, OnDestroy  {
  @ViewChild('editproductTable') table;
  ...
  constructor(private masterservice: MasterService, private featureservice: FeaturesService, private storageservice: LocalStorageService,
    ...
    private acroute: ActivatedRoute, private messageservice: MessagesService) {
    ...
    if (!_.isEmpty(this.billid) && this.billid != null && this.billid != undefined) {
    ...
    this.purchaseservice.getBillById(reqdata)
      .then(res => {
        if (res.status) {
          ...
          if(res.data[0].billtype == "ServiceInvoice")
          {
  ...
  formateditmode() {
    ...
    this.invoiceDate = this.dateFormatPipeFilter.transform(this.invoiceDeta.billdt, this.date_dformat);
  ...
  extractdeleteTaxIds() {
    this.deleteTaxIds = [];
    for (var i = 0; i < this.invoiceDeta.billTaxes.length; i++) {
      this.deleteTaxIds.push(this.invoiceDeta.billTaxes[i].billtaxid)
    }
  }
  ...
  formatProducts() {
    var products = this.invoiceDeta.billDetails;
  ...
  loadeditedinvoicetype() {
    this.selectedInvoiceTypeLabel = this.invoiceDeta.btype;
    this.selectedreversecharge = this.invoiceDeta.revcharge;
    var btype = _.find(this.Invoicetype, { "label": this.invoiceDeta.btype });
    if (!_.isEmpty(btype)) {
      this.selectedInvoiceType = btype.value;
  ...
  removeproduct(item, idx) {
    var osource = _.clone(this.selectedproductsList)
    osource.splice(osource.indexOf(item), 1);
    this.selectedproductsList = [...osource];
    this.calculateTotals();
  ...
  onProductSelect(event, selectedproduct, i, datatable: any) {
    var self = this;
    var index = self.selectedproductsList.indexOf(selectedproduct);
    var item = event.value;
  ...
  GenarateInvoice(flag) {
    ...
    if (typeof this.invoiceDate == "string") { // _.isEmpty(inDate)
      if (this.invoiceDate == "") {
        validdata = false;
        validationerrorMsg.push(PrimengConstant.INVOICE.ADDEDITFORM.INVOICEDATE);
      }
    }
    ...
    if (validdata) {
      ...
      var invdate = inDate || this.invoiceDate;
      var ddate = DueDate || this.DueDate;
      //    -------------------------------------------------------------- Acconfig Setup -------------------------------------------------------------------------
      _.forEach(this.accconfig, (config) => {
        ...
        config.ledgerdate = invdate;
        config.txnid = this.billid;
        config.feature = 'bill';
        config.finyear = this.finyear.finyear;
      ...
      var formData = {
        'header': {
          'billid': this.invoiceDeta.billid,
          'billno': this.invoiceDeta.billno,
          // "billno": this.invoiceNumber,
          'billdt': inDate,
          'finyear': this.finyear.finyear,
          'tenantid': this.userstoragedata.tenantid,
    ...
    if (validdata) {
      var self = this;
      this.purchaseservice.BillUpdate(formData)
        .then(function (res) {
          self.msgs = [];
          if (res.status) {
            ...
            // self.clearform();
            self.router.navigate(['purchase/viewbill', this.billid]);
  ...
  formatProductsForUpdate() {
    ...
    var newlyinserted = _.difference(this.selectedproductsList, this.invoiceDeta.billDetails);

    var deletedproducts = _.difference(this.invoiceDeta.billDetails, this.selectedproductsList);
    var updatedProduct = [];
