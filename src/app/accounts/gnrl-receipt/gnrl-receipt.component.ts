...
@Component({
  selector: 'app-gnrl-receipt',
  templateUrl: './gnrl-receipt.component.html',
  styleUrls: ['./gnrl-receipt.component.scss']
})
export class GnrlReceiptComponent implements OnInit {
  journalno: any;
  autogenyn = 'Y';
  ...
  initMenuItems() {
    ...
    this.updateMenuItems = [
      ...
      // },
      {
        label: 'Quit', icon: 'fa-minus', command: (event) => {
  ...
  switchToEditMode(queryParams) {
    this.journalService.getJournalDetails(queryParams.journalid + "/" + "gnrl_receipt")
      .then(response => {
        if (response.status) {
  ...
  selpayterm(item) {
    if (item != null && item != undefined) {
      ...
      this.featuresService.paymodeGetAll(data)
        .then((res) => {
          if (res.status) {
            ...
            console.log("Pay terms data: ", allpaymode);
          }
  ...
  assignToEditFormObj(formData: any) {
    ...
    let selectedAccountBooks: any = _.find(this.journalfeat, { value: { name: this.existingJournalDetails.type } });
    if (!_.isEmpty(selectedAccountBooks)) {
      this.selpayterm(this.existingJournalDetails.type);
    }
  ...
  onSelectAccountTypes(event, index, selectedFormGroup) {
    ...
    {
      selectedFormGroup.controls.credit.disable();
    selectedFormGroup.controls.debit.enable();
    selectedFormGroup.controls.debit.touched = true;
    }
    this.calcCrDrBalancing();
  }
        ...
        .then(res => {
          if (res.status == true) {
            if (action == 'view') {
              ...
              this.router.navigate(['accounts/viewtransactions/', journalId, journalNo, 'gnrl_receipt', 'receipt']);
      ...
      this.journalService.updateJournal(updateDetails)
        .then(res => {
          if (res.status) {
            ...
            if (action == 'view') {
              this.router.navigate(['accounts/viewtransactions/', this.paramDetails.journalId, this.paramDetails.journalNo, 'gnrl_receipt', 'receipt']);
            }
            else if (action == 'create') {
  ...
  prepareJournalHeaderDetails() {
    ...
    this.journalDetails.feature = "gnrl_receipt";
  ...
  prepareJournalDetails() {
    ...
    _.forEach(journalsEntry.controls, (control: FormGroup) => {
      if (!_.has(control.controls["account"].value, "value")) {
        ...
        journaldetails.feature = "gnrl_receipt";
