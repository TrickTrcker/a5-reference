...
@Component({
  selector: 'app-gnrl-payment',
  templateUrl: './gnrl-payment.component.html',
  styleUrls: ['./gnrl-payment.component.scss']
})
export class GnrlPaymentComponent implements OnInit {
  journalno: any;
  autogenyn = 'Y';
  ...
  switchToEditMode(queryParams) {
    this.journalService.getJournalDetails(queryParams.journalid + "/" + "gnrl_payment")
      .then(response => {
        if (response.status) {
        ...
        .then(res => {
          if (res.status == true) {
            if (action == 'view') {
              ...
              this.router.navigate(['accounts/viewtransactions/', journalId, journalNo, 'gnrl_payment', 'payment']);
            }
      ...
      this.journalService.updateJournal(updateDetails)
        .then(res => {
          if (res.status) {
            ...
            if (action == 'view') {
              this.router.navigate(['accounts/viewtransactions/', this.paramDetails.journalId, this.paramDetails.journalNo, 'gnrl_payment', 'payment']);
  ...
  prepareJournalHeaderDetails() {
    ...
    this.journalDetails.feature = "gnrl_payment";
  ...
  prepareJournalDetails() {
    ...
    _.forEach(journalsEntry.controls, (control: FormGroup) => {
      if (!_.has(control.controls["account"].value, "value")) {
        ...
        journaldetails.feature = "gnrl_payment";
