...
@Component({
  selector: 'app-bill-taxation',
  templateUrl: './bill-taxation.component.html',
  styleUrls: ['./bill-taxation.component.scss']
})
export class BillTaxationComponent implements OnInit {
  ...
  pdfdownload(contactdet, fromdate, todate) {
    let data = {} as any;
    data = this.UtilsService.getReportParams(AppConstant.REPORTS.BILL_TAXATION.REPORT_NAME, this.userstoragedata);
    ...
    var res = this.featureservice.reportDownload(data)
      ...
      data => {
        ...
        this.UtilsService.saveToFileSystem(data, "application/pdf", AppConstant.REPORTS.BILL_TAXATION.PREFIX_NAME, ".pdf");
        return data;
      },
      error => {
        console.error(error);
  ...
  getBillTaxation(frmdt, todt) {
    ...
    if (this.validation) {
      this.reportService.getBillTaxReport(data).then((res) => {
        this.list = res.data;
      })
    }
    else {
