...
export class OutstandingPaymentsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getoutstandingInv(this.currentDate, this.currentDate);
    this.getAllContacts();
  }
  pdfdownload(contactdet, fromdate, todate) {
    let data = {} as any;
    data = this.UtilsService.getReportParams(AppConstant.REPORTS.OUTSTANDING_PAY.REPORT_NAME, this.userstoragedata);
    ...
    var res = this.featureservice.reportDownload(data)
      .subscribe(
      data => {
        this.UtilsService.saveToFileSystem(data, "application/pdf", AppConstant.REPORTS.OUTSTANDING_PAY.PREFIX_NAME, ".pdf");
        return data;
      },
      error => {
        return error;
  ...
  getoutstandingInv(frmdt, todt) {
    ...
    if (this.validation) {
      this.reportService.getOutstandingPayment(data).then((res) => {
        this.list = res.data;
      })
    }
    else {
