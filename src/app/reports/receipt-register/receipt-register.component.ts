...
export class ReceiptRegisterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.getallreceipts(this.currentDate, this.currentDate);
    this.getAllContacts();
  }
  pdfdownload(contactdet, fromdate, todate) {
    let data = {} as any;
    data = this.UtilsService.getReportParams(AppConstant.REPORTS.RECEIPT_REGISTER.REPORT_NAME, this.userstoragedata);
    ...
    var res = this.featureservice.reportDownload(data)
      ...
      data => {
        console.log(data);
        this.UtilsService.saveToFileSystem(data, "application/pdf", AppConstant.REPORTS.RECEIPT_REGISTER.PREFIX_NAME, ".pdf");
        return data;
      },
      error => {
        console.error(error);
