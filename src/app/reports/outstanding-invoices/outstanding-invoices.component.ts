...
export class OutstandingInvoicesComponent implements OnInit {
  ...
  pdfdownload(contactdet, fromdate, todate) {
    let data = {} as any;
    data = this.UtilsService.getReportParams(AppConstant.REPORTS.OUTSTANDING_INV.REPORT_NAME, this.userstoragedata);
    ...
    var res = this.featureservice.reportDownload(data)
      .subscribe(
      data => {
        this.UtilsService.saveToFileSystem(data, "application/pdf", AppConstant.REPORTS.OUTSTANDING_INV.PREFIX_NAME, ".pdf");
        return data;
      },
      error => {
        return error;
  ...
  getoutstandingInv(frmdt, todt) {
    ...
    if (this.validation) {
      this.reportService.getOutstandingreport(data).then((res) => {
        this.list = res.data;
      })
    }
    else {
