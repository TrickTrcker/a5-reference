...
@Component({
  selector: 'app-creditnote-register',
  templateUrl: './creditnote-register.component.html',
  styleUrls: ['./creditnote-register.component.scss']
})
export class CreditnoteRegisterComponent implements OnInit {
  ...
  pdfdownload(contactdet, fromdate, todate) {
    let data = {} as any;
    data = this.UtilsService.getReportParams(AppConstant.REPORTS.CREDIT_NOTE_REG.REPORT_NAME, this.userdetails);
    ...
    var res = this.featureservice.reportDownload(data)
      .subscribe(
      data => {
        this.UtilsService.saveToFileSystem(data, "application/pdf", AppConstant.REPORTS.CREDIT_NOTE_REG.PREFIX_NAME, ".pdf");
        return data;
      },
      error => {
        return error;
