...
export class B2csComponent implements OnInit {
  ...
  constructor(private CommonService: CommonService, private route: ActivatedRoute,
    ...
    this.organisationsettingservice.TenantSettingList(data).then(
      res => {
        ...
        this.getB2CSList(this.selectedyear, this.download);
      });
  }

  ngOnInit() {
    this.filename = AppConstant.API_CONFIG.IDENTIFIER.B2CS + this.localstorageDetails.tenantid + '.xlsx';
  }
  ...
  getB2CSList(selectedyr, download) {
    this.data = {
      type: "GSTR1_B2CS", 
      finyear: selectedyr,
      pos: this.pos,
  ...
  ExporttoCSV() {
    if (this.listsize != 0 && this.listsize != undefined && this.listsize != '') {
      this.getB2CSList(this.selectedyear, 'Y');
    }
  }
